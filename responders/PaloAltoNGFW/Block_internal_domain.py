...
class Block_domain(Responder):
    ...
    def run(self):
        ...
        block_list = fw.find("TheHive Black list internal domain", panos.objects.AddressGroup)
        if block_list != None:
            ...
            temp1 = panos.objects.AddressGroup("TheHive Black list internal domain", static_value=ioc)
            fw.add(temp1)
            temp1.apply()
        ...
        for i in current_security_rules:
            if self.name_security_rule == i.about().get("name"):
                ...
                if "TheHive Black list internal domain" not in temp_rule_atrib:
                    temp_rule_atrib.append("TheHive Black list internal domain")
