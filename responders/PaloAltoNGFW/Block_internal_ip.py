...
class Block_ip(Responder):
    def __init__(self):
        ...
        self.name_security_rule = self.get_param('config.name_security_rule','TheHive Block internal IP address')
    ...
    def run(self):
        ...
        new_rule.apply()
        self.report({'message': 'Responder comlited, added %s into TheHive Black list internal IP from %s' % (ioc,self.name_security_rule)})
