...
class Block_domain(Responder):
    def __init__(self):
        ...
        self.name_security_rule = self.get_param('config.name_security_rule','TheHive Block external Domain')
    ...
    def run(self):
        ...
        block_list = fw.find("TheHive Black list external domain", panos.objects.AddressGroup)
        if block_list != None:
            ioc_list = block_list.about().get('static_value')
            if ioc not in ioc_list:
                ioc_list.append(ioc)
                temp1 = panos.objects.AddressGroup("TheHive Black list external domain", static_value=ioc_list)
                fw.add(temp1)
                temp1.apply()
        elif block_list == None:
            temp1 = panos.objects.AddressGroup("TheHive Black list external domain", static_value=ioc)
            fw.add(temp1)
            temp1.apply()
        ...
        for i in current_security_rules:
            if self.name_security_rule == i.about().get("name"):
                ...
                if "TheHive Black list external domain" not in temp_rule_atrib:
                    temp_rule_atrib.append("TheHive Black list external domain")
        ...
        new_rule.apply()
        self.report({'message': 'Responder comlited, aded %s into TheHive Black list external domain from %s' % (ioc,self.name_security_rule)})
