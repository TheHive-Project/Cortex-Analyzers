...
class Block_port(Responder):
    def __init__(self):
        ...
        self.name_security_rule = self.get_param('config.name_security_rule','TheHive Block internal port')
    ...
    def run(self):
        ...
        block_list = fw.find("TheHive Black list internal port", panos.objects.ServiceGroup)
        if block_list != None:
            port_list = block_list.about().get('value')
            if port not in port_list:
                port_list.append(port)
                temp1 = panos.objects.ServiceGroup("TheHive Black list internal port", value=port_list)
                fw.add(temp1)
                temp1.apply()
        elif block_list == None:
            temp1 = panos.objects.ServiceGroup("TheHive Black list internal port", value=port)
            fw.add(temp1)
            temp1.apply()
        ...
        new_rule.apply()
        self.report({'message': 'Responder comlited, added %s into TheHive Black list internal port from %s' % (port,self.name_security_rule)})
