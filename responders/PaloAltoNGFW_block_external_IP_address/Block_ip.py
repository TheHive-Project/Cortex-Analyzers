...
class Block_ip(Responder):
    def __init__(self):
        ...
        self.name_security_rule = self.get_param('config.name_security_rule','Block external IP address')
    ...
    def run(self):
        ...
        block_list = fw.find("Black list external IP", panos.objects.AddressGroup)
        if block_list != None:
            ioc_list = block_list.about().get('static_value')
            if ioc not in ioc_list:
                ioc_list.append(ioc)
                temp1 = panos.objects.AddressGroup("Black list external IP", static_value=ioc_list)
                fw.add(temp1)
                temp1.apply()
        elif block_list == None:
            temp1 = panos.objects.AddressGroup("Black list external IP", static_value=ioc)
            fw.add(temp1)
            temp1.apply()
        ...
        for i in current_security_rules:
            if self.name_security_rule == i.about().get("name"):
                ...
                if "Black list external IP" not in temp_rule_atrib:
                    temp_rule_atrib.append("Black list external IP")
                ...
                desired_rule_params = {
                    ...
                    "type": "interzone",
                    "action": "deny",
                    'destination': "Black list external IP"
                    }
